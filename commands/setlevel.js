...
module.exports = {
    name: 'setlevel',
    aliases: ["levelset", "setlvl", "lvlset"],
   ...
   if(togglexp !== 'on') return message.channel.send("This command is not toggled on!");
    ...
    const levelToSet = parseInt(args[1], 10);
    if(levelToSet) 
      return message.reply("put an amount of XP to give!")
if(isNaN(levelToSet)) return message.reply("that's not a number!")
    ...
    // And we save it!
    bot.points.set(`${message.guild.id}-${user.id}`, levelToSet, "points")
      
    message.channel.send(`Set successfully!\n${user} has **${levelToSet}** levels now.`);
  }
}
