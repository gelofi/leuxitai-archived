...
module.exports = {
  ...
  if(want === "changemymind"){
     ...
     try {
        let image = await canva.changemymind(args.slice(1).join(" "));
  ...
  let img = message.mentions.users.first() || message.attachments.first()
  if(!img) return message.reply("provide an image/@user to edit!")
    if(message.mentions.users.first()){
      img = message.mentions.users.first().avatarURL
    } else {
